...
export const RESEARCH_DIRECTION_SUMMARY_PILLARS: ResearchSummaryPillar[] = [
  {
    title: '突破前沿技术',
    body:
      '研究院计划重点突破多模态信息的智能处理、智能标识网络理论与构建方法，以及云边端一体化等前沿技术。这意味着未来的设备将能更智能地融合处理文本、图像、视频等多种信息，并实现云端、边缘端和设备端的无缝协同。',
  },
  {
    title: '打造AI新基座',
    body:
      '研究院发布了EdgeOS系统官网和埃启·云脑两款新品。其中，“埃启·云脑”旨在打造一个算法生态中枢，通过内置覆盖车辆检测、校园安全、智慧工业、医疗检测等场景的算法卡片库，实现算法的统一管理和便捷部署。这相当于构建了一个“算法商城”，目标是打通AI落地的“最后一公里”。',
  },
  {
    title: '攻关自主可控通信',
    body:
      '作为“新一代自主可控智能通信网络创新联合体”的重要成员，研究院将参与攻关多维标识驱动的自主可控通信协议栈、智能化通信机制与关键设备研制等核心技术。这响应了国家在关键领域解决“卡脖子”技术的需求，未来将应用于国防通信、智能制造等领域。',
  },
  {
    title: '融合大模型技术',
    body:
      '研究院明确将以“边缘智能和大模型为核心技术底座”，加速研发基于大模型的新一代人工智能服务平台。这表明其未来产品将具备更强大的泛化能力和智能水平。',
  },
];
...
export function flattenResearchSummary(): string {
  ...
  return [RESEARCH_DIRECTION_SUMMARY_LEAD, pillars, RESEARCH_DIRECTION_SUMMARY_OUTLOOK].join(
    nl,
  );
}
...
const PROVINCIAL_APPLY_URL =
  'https://jsszkj.kxjst.jiangsu.gov.cn/js-home/home';
    ...
    return {
      ...
      results: [
        ...
        },
        {
          ...
          url: 'https://example.com/search/achievement/scd-market',
        },
        {
          title: '哈尔滨工业大学金刚石材料与钎焊连接相关研究线索',
          source: '高校公开成果 / 专利信息检索',
          snippet:
            '哈工大在超硬材料、真空钎焊与界面工程方向具备长期积累，相关团队在籽晶工装、活性焊料配方与工艺参数优化方面有公开专利与论文线索。',
          url: 'https://example.com/search/achievement/hit-diamond-team',
        },
      ],
    };
  }
      ...
      results: [
        {
          ...
          url: 'https://example.com/search/research/edge-ai-1',
        },
        {
          ...
          url: 'https://example.com/search/research/edge-ai-2',
        },
        {
          ...
          url: 'https://example.com/search/research/edge-ai-3',
        },
      ],
    };
  }
...
export function buildPolicyRecommendResult(question: string): PolicyRecommendResult {
  return {
      ...
      provincial: [
        {
          ...
          item_name: '基础研究重点项目',
          level: '省级',
          funding_amount: '省市联合资助，单项目资助额度见申报通知（约不超过250万元）',
          item_category_description:
            '省基础研究计划项目（主管部门：省科技厅、省教育厅）',
          project_description: PROVINCIAL_PROJECT_DESC,
          application_requirements:
...
export function buildAchievementEvalResult(question: string): AchievementEvalResult {
  const dimensions = [
    {
      label: '创新性',
      score: 18,
